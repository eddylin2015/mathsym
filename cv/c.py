...
proximity_line_init_value = 170 - dino_HEAD
...
dino_top_off_set=  80 * scale_src_rate
### 
...
def game(thread_name, x, y, q):
    ...
    sct = mss()
    ##600x150
    monitor = {'top': y, 'left': x, 'width': 450*scale_src_rate, 'height': 140*scale_src_rate- dino_top_off_set}
    ...
    while True:
        ...
        for template in templates:
            #screen_roi<-screen_canny
            result = cv2.matchTemplate(screen_canny, template, cv2.TM_CCOEFF_NORMED)  # Match objects to screen
            ...
            if len(rectangles):
                ...
                for (x, y, w, h) in rectangles:
                    ...
                    if object_center < proximity:
                        ...
                        if (73-dino_top_off_set/scale_src_rate) < y < (83-dino_top_off_set/scale_src_rate):
                            # Duck, but not Jump
                            do_actions = [True, False]
                            q.put(do_actions)
                        elif y > (70-dino_top_off_set/scale_src_rate):
                            # Jump, but not Duck
